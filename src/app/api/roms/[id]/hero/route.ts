...
/** Set (or clear) a game's hero: downloads the chosen image locally */
export async function POST(
  ...
  if (url === null) {
    getDb().prepare("UPDATE roms SET hero_url = NULL WHERE id = ?").run(romId);
    logActivity({ userId: user.id, romId, type: "hero", summary: "Removed hero artwork" });
    return NextResponse.json({ ok: true, hero_url: null });
  }
  if (typeof url !== "string" || !url.trim()) {
    return NextResponse.json({ error: "url required (or null to clear)" }, { status: 400 });
  }

  // Local media (e.g. the current screenshot) can be referenced directly
  if (url.startsWith("/api/media/")) {
    getDb().prepare("UPDATE roms SET hero_url = ? WHERE id = ?").run(url, romId);
    logActivity({
      userId: user.id, romId, type: "hero",
      summary: "Updated hero artwork", imageSourcePath: mediaUrlToPath(url),
    });
    return NextResponse.json({ ok: true, hero_url: url });
  }
  if (!/^https?:\/\//i.test(url)) {
